    ...
    def train(self):
        for epoch in range(self.config['epoch'], self.config['n_epochs']):
            for i, batch in enumerate(self.dataloader):
                ...
                loss_GAN_fake_B = self.MSE_loss(pred_fake_B, self.target_real) * 2
                pred_fake_fake_B = self.netD_fake_fake_B(fake_fake_B) * 0.5
                loss_GAN_fake_fake_B = self.MSE_loss(pred_fake_fake_B, self.target_real)
                # 配准网络损失
                SR_fB_loss = self.config['Corr_lamda'] * self.L1_loss(Register_fake_B, real_B)
                ...
                # 总损失
                loss_Total = (loss_cycle_ABA + loss_GAN_fake_B + loss_GAN_fake_fake_B + SR_fB_loss + SM_fB_loss +
                              SR_ffB_loss + SM_ffB_loss + idt_B_loss + idt_A_loss)
                loss_Total.backward()
                ...
                self.logger.log({'loss_D_fake_B': loss_D_fake_B, 'loss_D_fake_fake_B': loss_D_fake_fake_B,
